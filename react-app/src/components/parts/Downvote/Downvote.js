...
const Downvote = ({ id, type, rating }) => {
  ...
  return (
    <>
      {rating === -1 ? (
        <button
          onClick={onUnDownvote}
          className="p-2 mx-2 border rounded border-purple text-purple active:outline-none focus:outline-none hover:border-gray-50 active:bg-none hover:text-gray-50"
        >
          <ArrowDownward />
        </button>
      ) : (
        <button
          onClick={onDownvote}
          className="p-2 mx-2 border rounded hover:border-purple active:bg-purple hover:text-purple"
        >
          <ArrowDownward />
        </button>
      )}
