...
export const useCollapsedSidebarContext = () => useContext(CollapsedSidebarContext);

const CollapsedSidebarProvider = ({ children }) => {
  const [showCollapsedSidebar, setShowCollapsedSidebar] = useState(false);

  return (
    <CollapsedSidebarContext.Provider value={{ showCollapsedSidebar, setShowCollapsedSidebar }}>
      { children }
